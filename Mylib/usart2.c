...
#define MIDDLE_VALUE       1024
#define VALUE_MIDDLE       460

#define FORWORD_SPEED      0.25
#define LEFT_RIGHT_SPEED   0.15

extern float target_speed[5];
extern float send_data[4];
void Control_Moto(unsigned short ch0,unsigned short ch1)
{
		if(ch1 > MIDDLE_VALUE+VALUE_MIDDLE)
		{
				target_speed[1] = FORWORD_SPEED;
				target_speed[2] = FORWORD_SPEED;
				target_speed[3] = FORWORD_SPEED;
				target_speed[4] = FORWORD_SPEED;
		}
		if(ch1 < MIDDLE_VALUE-VALUE_MIDDLE)
		{
				target_speed[1] = -FORWORD_SPEED;
		...
		if(ch0 > MIDDLE_VALUE+VALUE_MIDDLE)
		{
				target_speed[1] = -LEFT_RIGHT_SPEED;
				target_speed[2] = LEFT_RIGHT_SPEED;
				target_speed[3] = LEFT_RIGHT_SPEED;
				target_speed[4] = -LEFT_RIGHT_SPEED;
		}
		if(ch0 < MIDDLE_VALUE -VALUE_MIDDLE)
		{
				target_speed[1] = LEFT_RIGHT_SPEED;
...
*/
		...
		Control_Moto(RC_Ctl.rc.ch0,RC_Ctl.rc.ch1);
}
